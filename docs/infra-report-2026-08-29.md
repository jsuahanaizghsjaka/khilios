...
1. Токен `@khilios_vpn_bot` ранее попадал в чат. Его нужно отозвать в
   `@BotFather` и выдать новый. На рабочем компьютере нет авторизованного
   Telegram Desktop/Web, поэтому безопасно завершить этот пункт автоматически
   невозможно. Новый токен нельзя присылать в чат: его следует сразу записать
   в `/opt/khilios/infra/bot/bot.env`, затем пересобрать бот.
